...
##########################################################################
*/
if (isset($site)) $_language->read_module('awards');

if(isset($_POST['save'])) {
	include("_mysql.php");
	include("_settings.php");
	include("_functions.php");
	$_language->read_module('awards');
	...
	safe_query("INSERT INTO ".PREFIX."awards ( date, squadID, award, homepage, rang, info ) VALUES( '$date', '$squad', '$award', '$homepage', '$rang', '$info' ) ");
...
elseif(isset($_POST['saveedit'])) {
	...
	$date=mktime(0,0,0,$month,$day,$year);
	safe_query("UPDATE ".PREFIX."awards SET date='$date', squadID='$squad', award='$award', homepage='$homepage', rang='$rang', info='$info' WHERE awardID='$awardID'");
	header("Location: index.php?site=awards");
}
elseif(isset($_GET['delete'])) {
	...
	safe_query("DELETE FROM ".PREFIX."awards WHERE awardID='$awardID'");
	header("Location: index.php?site=awards");
}
...
if($action=="new") {
	...
	else redirect('index.php?site=awards', $_language->module['no_access']);
}
elseif($action=="edit") {
...
elseif($action=="showsquad") {
	...
	if($pages>1) $page_link = makepagelink("index.php?site=awards&amp;action=showsquad&amp;squadID=$squadID&amp;sort=$sort&amp;type=$type", $page, $pages);
	else $page_link = "";
	if ($page == "1") {
		$ergebnis = safe_query("SELECT * FROM ".PREFIX."awards WHERE squadID='$squadID' ORDER BY $sort $type LIMIT 0,$max");
		if($type=="DESC") $n=$gesamt;
		else $n=1;
	}
	else {
		$start=$page*$max-$max;
		$ergebnis = safe_query("SELECT * FROM ".PREFIX."awards WHERE squadID='$squadID' ORDER BY $sort $type LIMIT $start,$max");
  ...
  $alle=safe_query("SELECT awardID FROM ".PREFIX."awards");
	...
	if($pages>1) $page_link = makepagelink("index.php?site=awards&sort=$sort&type=$type", $page, $pages);
	else $page_link = '';
	...
	if($gesamt) {
		if($type=="ASC")
		echo'<a href="index.php?site=awards&amp;page='.$page.'&amp;sort='.$sort.'&amp;type=DESC">'.$_language->module['sort'].':</a> <i class="icon-sort-down"></i>';
		else
		...
		while($ds=mysqli_fetch_array($ergebnis)) {
			...
			if(isclanwaradmin($userID) || isnewsadmin($userID))
			$adminaction='<input type="button" class="btn btn-danger" onclick="MM_goToURL(\'parent\',\'index.php?site=awards&amp;action=edit&amp;awardID='.$ds['awardID'].'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" />
      <input type="button" class="btn btn-danger" onclick="MM_confirm(\'really delete this award?\',\'index.php?site=awards&amp;delete=true&amp;awardID='.$ds['awardID'].'\')" value="'.$_language->module['delete'].'" />';
			else $adminaction = '';

...
function getuserforumtopics($userID) {
	$anz=mysqli_num_rows(safe_query("SELECT topicID FROM ".PREFIX."forum_topics WHERE userID='$userID' "));
	return $anz;
}

function getuserforumposts($userID) {
	$anz=mysqli_num_rows(safe_query("SELECT postID FROM ".PREFIX."forum_posts WHERE poster='$userID'"));
	return $anz;
}
...
function usergrpexists($fgrID) {
	$anz=mysqli_num_rows(safe_query("SELECT fgrID FROM ".PREFIX."forum_groups WHERE fgrID='$fgrID'"));
	return $anz;
}
function boardexists($boardID){
	$anz = mysqli_num_rows(safe_query("SELECT name FROM ".PREFIX."forum_boards WHERE boardID='$boardID'"));
	return $anz;
}
?>
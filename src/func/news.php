...
{
    return mysqli_num_rows(safe_query("SELECT newsID FROM `" . PREFIX . "news` WHERE `poster` = " . (int)$userID));
}
...
function getrubricname($rubricID)
{
    $ds = mysqli_fetch_array(
        safe_query(
            "SELECT rubric FROM `" . PREFIX . "news_rubrics` WHERE `rubricID` = " . (int)$rubricID
        )
    );
    return $ds['rubric'];
}
...
function getrubricpic($rubricID)
{
    $ds = mysqli_fetch_array(
        safe_query(
            "SELECT pic FROM `" . PREFIX . "news_rubrics` WHERE `rubricID` = " . (int)$rubricID
        )
    );
    return $ds['pic'];
}

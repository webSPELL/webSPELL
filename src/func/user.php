...
function usergroupexists($userID) {
	return (mysqli_num_rows(safe_query("SELECT userID FROM ".PREFIX."user_groups WHERE userID='".$userID."'")) > 0);
}

function wantmail($userID) {
	return (mysql_num_rows(safe_query("SELECT userID FROM ".PREFIX."user WHERE userID='".$userID."' AND mailonpm='1'")) > 0);
}
...
function getusercomments($userID, $type) {
	return mysqli_num_rows(safe_query("SELECT commentID FROM `".PREFIX."comments` WHERE userID='".$userID."' AND type='".$type."'"));
}

function getallusercomments($userID){
	return mysqli_num_rows(safe_query("SELECT commentID FROM `".PREFIX."comments` WHERE userID='".$userID."'"));
}

function isbuddy($userID, $buddy) {
	return (mysqli_num_rows(safe_query("SELECT * FROM ".PREFIX."buddys WHERE banned=0 AND buddy=".(int)$buddy." AND userID=".(int)$userID)) > 0);
}

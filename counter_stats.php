...
for($i = date("d",time());$i>0;$i--) {
	...
	$tmp = mysqli_fetch_array(safe_query("SELECT count FROM ".PREFIX."counter_stats WHERE dates='".$i.$datemonth."'"));
...
$online_lasthour = mysqli_num_rows(safe_query("SELECT ip FROM ".PREFIX."counter_iplist WHERE del>".(time()-3600)));
...
$get = mysqli_fetch_assoc(safe_query("SELECT ROUND(SUM(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW()) - TO_DAYS(birthday)), '%y')) / COUNT(userID)) as 'avg_age' FROM ".PREFIX."user WHERE birthday > 0"));

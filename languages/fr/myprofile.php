<?php
/*
##########################################################################
#                                                                        #
#           Version 4       /                        /   /               #
#          -----------__---/__---__------__----__---/---/-               #
#           | /| /  /___) /   ) (_ `   /   ) /___) /   /                 #
#          _|/_|/__(___ _(___/_(__)___/___/_(___ _/___/___               #
#                       Free Content / Management System                 #
#                                   /                                    #
#                                                                        #
#                                                                        #
#   Copyright 2005-2015 by webspell.org                                  #
#                                                                        #
#   visit webSPELL.org, webspell.info to get webSPELL for free           #
#   - Script runs under the GNU GENERAL PUBLIC LICENSE                   #
#   - It's NOT allowed to remove this copyright-tag                      #
#   -- http://www.fsf.org/licensing/licenses/gpl.html                    #
#                                                                        #
#   Code based on WebSPELL Clanpackage (Michael Gruber - webspell.at),   #
#   Far Development by Development Team - webspell.org                   #
#                                                                        #
#   visit webspell.org                                                   #
#                                                                        #
##########################################################################
*/

$language_array = Array(

/* do not edit above this line */

    'about_myself' => 'À propos de moi',
    'activated' => 'activé',
    'avatar' => 'Avatar:',
    'back' => 'Retour',
    'bbcode' => '<a href="javascript:void(0);" onclick="window.open(\'code.php\',\'BBCode\',\'scrollbars=yes,width=600,height=500\')">BBCode</a> activé',
    'can_not_copy' => 'Impossible de copier le fichier sur le serveur',
    'change_mail' => 'Enregistrer',
    'change_password' => 'Enregistrer',
    'clan-history' => 'Anciennes teams',
    'clan-hp' => 'Site web',
    'clan-info' => 'Informations sur le clan',
    'clan-irc' => 'Irc',
    'clan-name' => 'Nom',
    'clan-tag' => 'Tag',
    'country' => 'Pays',
    'cpu' => 'Processeur',
    'date_of_birth' => 'Date de naissance',
    'deactivated' => 'désactivé',
    'delete' => 'delete',
    'delete_max_90_90' => 'Supprimer (max. 90/90)',
    'delete_max_230_210' => 'Supprimer (max. 230/210)',
    'e-mail' => 'Email',
    'edit_mail' => 'Modifier l\'adresse mail',
    'edit_password' => 'Modifier le mot de passe',
    'equipment-info' => 'Informations sur le matériel',
    'errors_there' => 'Erreurs',
    'female' => 'Féminin',
    'fields_star_required' => 'Les champs marqués avec * sont obligatoires',
    'first_name' => 'Prénom',
    'forgot_new_pw' => 'Vous avez oublié d\'entrer votre nouveau mot de passe !',
    'forgot_old_pw' => 'Vous avez oublié d\'entrer votre ancien mot de passe !',
    'format_date' => 'Format de la date',
    'format_time' => 'Format de l\'heure',
    'graphiccard' => 'Carte graphique',
    'guestbook' => 'Livre d\'or',
    'hdd' => 'Disque dur',
    'headset' => 'Casque à écouteurs',
    'hide_e-mail' => 'Cacher l\'email ?',
    'homepage' => 'Site web',
    'html' => 'HTML désactivé',
    'i-connection' => 'Connexion internet',
    'invalid_mail' => 'Vous avez entrer une adresse mail invalide !',
    'invalid_picture-format' => 'Format d\'image invalide (autorisés: *.gif, *.jpg ou *.png)',
    'keyboard' => 'Clavier',
    'language' => 'Langue',
    'last_name' => 'Nom',
    'mail_changed' => 'Votre adresse mail a été changé. Vous allez recevoir un mail avec un lien d\'activation dans quelques instants. Jusqu\'à ce que vous activiez votre nouvelle adresse mail, l\'ancienne adresse restera valide.',
    'mail_failed' => 'Le mail d\'activation n\'a pas pu être envoyé, veuillez en informer le webmaster.',
    'mail_not_valid' => 'Vos adresses mails sont différentes !',
    'mail_on_new_pm' => 'Envoyer un mail lors d\'un nouveau message privé ?',
    'mail_subject' => 'Mail d\'activation de compte pour %homepage_url%',
    'mail_text' => 'Bienvenue %username%!

Vous avez changé d\'adresse mail sur %pagetitle% (%homepage_url%) avec succès !

Pour compléter votre changement d\'adresse mail, vous devez activer votre adresse mail en cliquant sur le lien suivant:

%activationlink%

Merci pour votre intérêt
%pagetitle% - %homepage_url%',
    'mainboard' => 'Carte mère',
    'male' => 'Masculin',
    'monitor' => 'Ecran',
    'mouse' => 'Souris',
    'mousepad' => 'Tapis de souris',
    'my_profile' => 'Mon profil',
    'new_email' => 'Nouvelle adresse mail',
    'new_password' => 'Nouveau mot de passe',
    'newsletter' => 'Newsletter ?',
    'nickname' => 'Pseudo',
    'nickname_already_in_use' => 'Pseudo déjà utilisé !',
    'no' => 'Non',
    'not_logged_in' => 'Vous devez être connecté pour pouvoir éditer votre profil !<br><br>
    &#8226; <a href="index.php?site=register">S\'enregistrer</a><br>
    &#8226; <a href="index.php?site=login">Connexion</a>',
    'old_password' => 'Ancien mot de passe',
    'old_pw_not_valid' => 'Votre ancien mot de passe n\`est pas valide !',
    'options' => 'Options',
    'or' => 'ou',
    'password' => 'Mot de passe',
    'personal-info' => 'Informations personnelles',
    'picture_too_big_avatar' => 'L\'image est trop grande (max. 90/90px)',
    'picture_too_big_userpic' => 'L\'image est trop grande (max. 230/210px)',
    'profile-info' => 'Informations de profil',
    'profile_updated' => 'Votre profil a été mis à jour. Patientez quelques secondes pour être redirigé !',
    'pw_changed' => 'Votre mot de passe a été changé. Vous devez vous reconnecter.',
    'ram' => 'Mémoire',
    'repeat_new_email' => 'Répéter la nouvelle adresse mail',
    'repeat_new_password' => 'Répéter le nouveau mot de passe',
    'repeated_pw_not_valid' => 'Vos mots de passe sont différents !',
    'sexuality' => 'Sexe',
    'signature' => 'Signature',
    'smilies' => '<a href="javascript:void(0);" onclick="window.open(\'smileys.php\',\'Smileys\',\'scrollbars=yes,width=340,height=500\')">Smileys</a> activés',
    'soundcard' => 'Carte son',
    'town' => 'Ville',
    'unknown' => 'Non indiqué',
    'update_profile' => 'Enregistrer',
    'upload_failed' => 'L\'envoi a échoué',
    'username' => 'Nom d\'utilisateur',
    'username_aleady_in_use' => 'Vous avez indiqué un nom d\'utilisateur déjà utilisé !',
    'userpic' => 'Photo:',
    'yes' => 'Oui',
    'you_have_to_bday' => 'Vous devez entrer votre date d\'anniversaire correctement !',
    'you_have_to_email' => 'Vous devez entrer votre adresse mail !',
    'you_have_to_firstname' => 'Vous devez entrer votre prénom !',
    'you_have_to_nickname' => 'Vous devez entrer un pseudo !',
    'you_have_to_username' => 'Vous devez entrer un nom d\'utilisateur !',
    'you_have_to_valid_email' => 'Votre adresse mail n\'est pas valide !'
);

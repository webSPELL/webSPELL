<?php
/*
##########################################################################
#                                                                        #
#           Version 4       /                        /   /               #
#          -----------__---/__---__------__----__---/---/-               #
#           | /| /  /___) /   ) (_ `   /   ) /___) /   /                 #
#          _|/_|/__(___ _(___/_(__)___/___/_(___ _/___/___               #
#                       Free Content / Management System                 #
#                                   /                                    #
#                                                                        #
#                                                                        #
#   Copyright 2005-2010 by webspell.org                                  #
#                                                                        #
#   visit webSPELL.org, webspell.info to get webSPELL for free           #
#   - Script runs under the GNU GENERAL PUBLIC LICENSE                   #
#   - It's NOT allowed to remove this copyright-tag                      #
#   -- http://www.fsf.org/licensing/licenses/gpl.html                    #
#                                                                        #
#   Code based on WebSPELL Clanpackage (Michael Gruber - webspell.at),   #
#   Far Development by Development Team - webspell.org                   #
#                                                                        #
#   visit webspell.org                                                   #
#                                                                        #
##########################################################################
*/


$language_array = Array(

/* do not edit above this line */

  'all_articles'=>'Все статьи',
  'already_rated'=>'<b>Вы уже оценили статью!</b>',
  'articles'=>'Статьи',
  'author'=>'Автор',
  'bbcode'=>'&#8226; <a href="javascript:MM_openBrWindow(\'code.php\',\'BBCode\',\'scrollbars=yes,width=600,height=500\')">BBКод</a> <b><u>включено</u></b>',
  'cancel'=>'Отменить',
  'date'=>'Дата',
  'delete'=>'Удалить',
  'edit'=>'Редактировать',
  'edit_article'=>'Редактировать статью',
  'enter_title'=>'Вы должны ввести название!',
  'enter_text'=>'Вы должны ввести текст',
  'html'=>'&#8226; HTML <b><u>включено</u></b>',
  'link'=>'Ссылка',
  'links'=>'Ссылки',
  'login_for_rate'=>'<b>Вы должны быть зарегестрированны и зайти за свой аккаунт, чтобы оценить статью!</b>',
  'new_article'=>'Новая статья',
  'new_window'=>'В новом окне',
  'no_access'=>'Доступ запрещён',
  'no_comments'=>'Отключить комментарии',
  'no_entries'=>'Статей нет',
  'options'=>'Опции',
  'page'=>'Страница',
  'page_of'=>'Страница $page из $pages',
  'page_select'=>'Разбить статью на <select name="language_count" onchange="update_textarea(this.options[this.selectedIndex].value)">$selects</select> страниц',
  'pagecount'=>'Количество страниц',
  'perfect'=>'Отлично',
  'poor'=>'Плохо',
  'rate'=>'Оценка',
  'rate_with'=>'Оцените, максимальный балл 10',
  'rating'=>'Рейтинг',
  'really_delete'=>'вы действительно хотите удалить статью?',
  'save_article'=>'Сохранить статью',
  'self'=>'в том же окне',
  'smilies'=>'&#8226; <a href="javascript:MM_openBrWindow(\'smileys.php\',\'Smileys\',\'scrollbars=yes,width=340,height=500\')">Смайлы</a> <b><u>включено</u></b>',
  'sort'=>'Сортировать:',
  'title'=>'Наслов',
  'top5_points'=>'Топ 5 статей (по очкам)',
  'top5_rating'=>'Топ 5 статей (по рейтингу)',
  'upload_images'=>'Загрузить изображения',
  'user_comments'=>'Разрешить комментировать зарегестированным пользователям',
  'viewed'=>'Просмотры',
  'visitor_comments'=>'Разрешить комментировать не зарегестрированным пользователям',
  'tags'=>'Метки'
);
?>
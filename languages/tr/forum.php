<?php
/*
##########################################################################
#                                                                        #
#           Version 4       /                        /   /               #
#          -----------__---/__---__------__----__---/---/-               #
#           | /| /  /___) /   ) (_ `   /   ) /___) /   /                 #
#          _|/_|/__(___ _(___/_(__)___/___/_(___ _/___/___               #
#                       Free Content / Management System                 #
#                                   /                                    #
#                                                                        #
#                                                                        #
#   Copyright 2005-2010 by webspell.org                                  #
#                                                                        #
#   visit webSPELL.org, webspell.info to get webSPELL for free           #
#   - Script runs under the GNU GENERAL PUBLIC LICENSE                   #
#   - It's NOT allowed to remove this copyright-tag                      #
#   -- http://www.fsf.org/licensing/licenses/gpl.html                    #
#                                                                        #
#   Code based on WebSPELL Clanpackage (Michael Gruber - webspell.at),   #
#   Far Development by Development Team - webspell.org                   #
#                                                                        #
#   visit webspell.org                                                   #
#                                                                        #
##########################################################################
*/
/**
 * Online Language Editor Turkey language file for webSPELL 4.2.2
 * @author Stefan Giesecke <webmaster@giesi.com>
 * @copyright Copyright (c) 2010, Stefan Giesecke
 * @license http://www.gnu.org/licenses/gpl.html GNU General Public License v3
 */

$language_array = Array(

/* do not edit above this line */

  'add_buddy'=>'arkadaş listesine ekle',
  'admin'=>'Yönetici',
  'admin_actions'=>'Yönetici-Eylemleri',
  'and'=>'ve',
  'author'=>'Yazar',
  'back_buddy'=>'arkadaş listesine geri dön',
  'bbcode_on'=>'<a href="javascript:MM_openBrWindow(\'code.php\',\'BBCode\',\'scrollbars=yes,width=600,height=500\')">BBCode</a> AÇIK',
  'board'=>'Pano',
  'boards'=>'Panolar',
  'clanmember'=>'Klan Üyeleri',
  'close_topic'=>'konuyu Kapat',
  'closed'=>'kapatıldı',
  'closed_image'=>'<img src="images/icons/closed.gif" alt="" />',
  'default_topic_title'=>'(başlık yok)',
  'delete_posts'=>'seçili mesajları sil',
  'delete_topic'=>'konuyu sil',
  'edit'=>'düzenle',
  'edit_reply'=>'yanıtı düzenle',
  'edit_topic'=>'konuyu düzenle',
  'forgot_message'=>'Bir mesaj girmeyi unuttunuz!',
  'go'=>'GİT',
  'guest'=>'1 konuk',
  'guests'=>'konuklar',
  'happy_birthday'=>'Mutlu Yıllar!',
  'homepage'=>'Anasayfa',
  'html_off'=>'HTML KAPALI',
  'ignore'=>'kulllanıcıyı yoksay',
  'lastpost'=>'Son Mesaj',
  'latest_post'=>'En Son Mesaj',
  'leave_shadowtopic'=>'Shadowtopic ten Ayrıl?',
  'make_sticky'=>'Bu konuyu yapışkan yapın',
  'make_topic_sticky'=>'bu konuyu yapışkan yapın',
  'make_topic_unsticky'=>'bu konuyu yapışkan yapmayın',
  'mark_all_read'=>'tüm panoları okundu olarak işaretle',
  'mark_topics_read'=>'tüm konuları okundu olarak işaretle',
  'maximum_online'=>'Maksimum çevrimiçi',
  'message'=>'Mesajlar',
  'messageboard'=>'Mesaj Panosu',
  'messenger'=>'Messenger',
  'moderated_by'=>'Moderatörlüğünü',
  'moderator'=>'Moderator',
  'move_topic'=>'konuyu taşı',
  'move_topic_board'=>'kuruluna bu konuyu taşı',
  'moved'=>'taşındı',
  'n_a'=>'n/a',
  'new_posts'=>'yeni Mesajlar',
  'new_reply'=>'yeni Yanıt',
  'new_topic'=>'yeni Konu',
  'newest_member'=>'yeni Üye',
  'newreply_image'=>'<img src="images/icons/newreply.gif" border="0" alt="reply" />',
  'newtopic_image'=>'<img src="images/icons/newtopic.gif" border="0" alt="new topic" />',
  'next_7_days'=>'sonraki 7 gün',
  'no_access'=>'Erişim Yok',
  'no_access_write'=>'Bu mesaj panosuna yazmak için erişim yok.',
  'no_icon'=>'simge yok',
  'no_new_posts'=>'Yeni mesaj yok',
  'no_permission'=>'stajyer panoları görmek için izniniz yok!',
  'not_logged'=>'Giriş yapmadınız.',
  'not_logged_msg'=>'<br><br>Konuya cevap vermek için kayıt yapman gerek!<br><br>
&#8226; <a href="index.php?site=register">şimdi üye ol</a><br>
&#8226; <a href="index.php?site=login">giriş</a>',
  'notify'=>'Bir yanıt geldiğinde bana bildir',
  'notify_mail'=>'Yeni cevap tarafından %poster% konunun içinde
%topic_link%

%pagetitle% - %hpurl%',
  'notify_reply'=>'Bir yanıt geldiğinde bana bildir',
  'now_online'=>'Şimdi Çevrimiçi',
  'options'=>'Seçenekler',
  'permission_denied'=>'İzin reddedildi!',
  'popular'=>'Popüler',
  'post_new_reply'=>'mesaj yeni yanıt',
  'post_new_topic'=>'yeni Konu',
  'posts'=>'Mesajlar',
  'posts_in'=>'mesajın icindeki',
  'preview'=>'Önizleme',
  'preview_post'=>'yazımın önizlemesi',
  'quote'=>'alıntı',
  'read_only'=>'sadece oku',
  'registered'=>'kayıtlı',
  'registered_user'=>'1 kayıtlı üye',
  'registered_users'=>'kayıtlı kullanıcılar',
  'reopen_topic'=>'konuyu yeniden',
  'replies'=>'Cevaplar',
  'search_image'=>'<img src="images/icons/search.gif" border="0" alt="" />',
  'select_all'=>'tümünü seç',
  'smilies_on'=>'<a href="javascript:MM_openBrWindow(\'smileys.php\',\'Smileys\',\'scrollbars=yes,width=340,height=500\')">İfadeler</a> AÇIK',
  'sort'=>'Tür',
  'stats'=>'İstatistikler',
  'sticky'=>'yapışkan',
  'subject'=>'Tema',
  'today'=>'bugün',
  'topic'=>'Konu',
  'topic_closed'=>'Konu kapatıldı!',
  'topic_icon'=>'konu-simge',
  'topic_not_found'=>'Konu bulunamadı.',
  'topics'=>'Konular',
  'views'=>'İzlenme',
  'who_was_online'=>'Kimler çevrimiçiydi',
  'whos_online'=>'Kimler çevrimiçi?',
  'wrote'=>'yazdı',
  'yesterday'=>'dün',
  'enter_topic'=>'Bir konu başlığı girmek zorundasınız!',
  'enter_text'=>'Bazı metin girmek zorundasınız!'
);
?>
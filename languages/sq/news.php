<?php
/*
##########################################################################
#                                                                        #
#           Version 4       /                        /   /               #
#          -----------__---/__---__------__----__---/---/-               #
#           | /| /  /___) /   ) (_ `   /   ) /___) /   /                 #
#          _|/_|/__(___ _(___/_(__)___/___/_(___ _/___/___               #
#                       Free Content / Management System                 #
#                                   /                                    #
#                                                                        #
#                                                                        #
#   Copyright 2005-2015 by webspell.org                                  #
#                                                                        #
#   visit webSPELL.org, webspell.info to get webSPELL for free           #
#   - Script runs under the GNU GENERAL PUBLIC LICENSE                   #
#   - It's NOT allowed to remove this copyright-tag                      #
#   -- http://www.fsf.org/licensing/licenses/gpl.html                    #
#                                                                        #
#   Code based on WebSPELL Clanpackage (Michael Gruber - webspell.at),   #
#   Far Development by Development Team - webspell.org                   #
#                                                                        #
#   visit webspell.org                                                   #
#                                                                        #
##########################################################################
*/

$language_array = array(

/* do not edit above this line */

    'author' => 'Autor',
    'bbcode' => 'BBCode është <b><span class="underline">NDEZUR</span></b>',
    'cancel' => 'anullo',
    'comm' => 'koment',
    'comment' => '<a href="$url">[1] comment</a>, të fundit nga $lastposter - $lastdate',
    'comments' => '<a href="$url">[$anzcomments] comments</a>, të fundit nga $lastposter - $lastdate',
    'date' => 'Data',
    'delete' => 'fshij',
    'delete_selected' => 'fshiji të zgjedhurat',
    'edit' => 'edito',
    'enter_text' => 'Duhet të vendosni një tekst',
    'enter_title' => 'Duhet të vendosni një titull!',
    'go' => 'Jepi!',
    'headline' => 'Kryeartikull',
    'html' => 'HTML është <b><span class="underline">NDEZUR</span></b>',
    'intern' => 'i brendshëm',
    'languages' => 'Gjuhët',
    'link' => 'Link',
    'links' => 'Link-et',
    'new_post' => 'Postim i ri',
    'new_window' => 'dritare e re',
    'news' => 'Lajme',
    'news_archive' => 'Arkiv',
    'no' => 'jo',
    'no_access' => 'ndalohet hyrja',
    'no_comment' => '<a href="$url">s\'ka komente</a>',
    'no_comments' => 'ç\'aktivizo komentet',
    'no_topnews' => 'nuk ka lajme kryesore',
    'options' => 'opsione',
    'post_languages' => 'Lajmet në<select name="language_count" onchange="update_textarea(this.options[this.selectedIndex].value)">$selects</select> gjuhët',
    'post_news' => 'posto të rejat',
    'preview' => 'parashikim',
    'publish_now' => 'publiko tani',
    'publish_selected' => 'publiko të zgjedhurat',
    'really_delete' => 'vërtet e fshin këtë lajm?',
    'rubric' => 'Rubrika',
    'save_news' => 'shpëto lajmet',
    'select_all' => 'zgjidhi të gjitha',
    'self' => 'dritare vetë',
    'show_news' => 'trego lajmet',
    'smilies' => 'Smilies janë <b><span class="underline">NDEZUR</span></b>',
    'sort' => 'Grupo:',
    'tags' => 'tags',
    'title_unpublished_news' => '<h2>LAJME TE PAPUBLIKUARA:</h2>',
    'topnews' => 'lajmet kryesore:',
    'unpublish' => 'ç\'publiko',
    'unpublish_selected' => 'ç\'publiko të zgjedhurat',
    'unpublished_news' => 'lajme të papublikuara',
    'upload_images' => 'upload imazhet',
    'user_comments' => 'lejo komentet e përdoruesve',
    'view_more' => 'Shih më tepër...',
    'visitor_comments' => 'lejo komentet e vizituesve',
    'written_by' => 'shkruar nga',
    'yes' => 'po'
);


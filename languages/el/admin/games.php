<?php
/*
##########################################################################
#                                                                        #
#           Version 4       /                        /   /               #
#          -----------__---/__---__------__----__---/---/-               #
#           | /| /  /___) /   ) (_ `   /   ) /___) /   /                 #
#          _|/_|/__(___ _(___/_(__)___/___/_(___ _/___/___               #
#                       Free Content / Management System                 #
#                                   /                                    #
#                                                                        #
#                                                                        #
#   Copyright 2005-2015 by webspell.org                                  #
#                                                                        #
#   visit webSPELL.org, webspell.info to get webSPELL for free           #
#   - Script runs under the GNU GENERAL PUBLIC LICENSE                   #
#   - It's NOT allowed to remove this copyright-tag                      #
#   -- http://www.fsf.org/licensing/licenses/gpl.html                    #
#                                                                        #
#   Code based on WebSPELL Clanpackage (Michael Gruber - webspell.at),   #
#   Far Development by Development Team - webspell.org                   #
#                                                                        #
#   visit webspell.org                                                   #
#                                                                        #
##########################################################################
*/

$language_array = Array(

/* do not edit above this line */

  'access_denied'=>'Δεν επιτρέπεται η πρόσβαση',
  'actions'=>'Ενεργειες',
  'add_game'=>'προσθέστε Παιχνίδι',
  'back'=>'πίσω',
  'delete'=>'διαγραφη',
  'edit'=>'Επεξεργασία',
  'edit_game'=>'επεξεργαστείτε Παιχνιδιού',
  'fill_correctly'=>'Παρακαλούμε συμπληρώστε τη φόρμα σωστά.',
  'format_incorrect'=>'Η μορφή του εικονιδίου ήταν εσφαλμένη. Παρακαλούμε ανεβάστε μόνο εικόνες σε μορφή * .gif.',
  'game_icon'=>'Game Icon',
  'game_name'=>'Όνομα Παιχνιδιού',
  'game_tag'=>'Ετικέτα Παιχνίδιου',
  'games'=>'Παιχνίδια',
  'icons'=>'Εικονίδια',
  'new_game'=>'νέο Παιχνίδι',
  'no_entries'=>'χωρις καταχωρησεις',
  'present_icon'=>'παρών Εικονίδιο',
  'really_delete'=>'θελετε πραγματικά διαγράψετε αυτό το παιχνίδι;',
  'transaction_invalid'=>'Μη έγκυρο ΑΝΑΓΝΩΡΙΣΤΙΚΌ συναλλαγής'
);


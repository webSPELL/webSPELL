...
function vote($poll) {
	...
	if($poll) $lastpoll = safe_query("SELECT * FROM ".PREFIX."poll WHERE aktiv='1' AND laufzeit>".time()." AND (intern = false OR intern = ".isclanmember($userID).") AND pollID='".$poll."' LIMIT 0,1");
	else {
		$num = mysqli_num_rows(safe_query("SELECT * FROM ".PREFIX."poll WHERE aktiv='1' AND laufzeit>".time()." AND intern<=".(int)isclanmember($userID).""));
		if($num) {
			$start = rand(0,($num-1));
			$lastpoll = safe_query("SELECT * FROM ".PREFIX."poll WHERE aktiv='1' AND laufzeit>".time()." AND intern<=".(int)isclanmember($userID)." ORDER BY pollID DESC LIMIT ".$start.",".($start+1)."");
		}
		else {
	...
	if($anz) {
		...
		$anz = mysqli_num_rows(safe_query("SELECT pollID FROM `".PREFIX."poll` WHERE pollID='".$ds['pollID']."' AND hosts LIKE '%".$_SERVER['REMOTE_ADDR']."%' AND intern<=".(int)isclanmember($userID).""));
		$anz_user = false;
		if($userID) {

...
if(isset($site)) $_language->read_module('cashbox');

if(isset($_POST['save']) and $_POST['save']) {
	include("_mysql.php");
	include("_settings.php");
	include("_functions.php");
	$_language->read_module('cashbox');
	if(!iscashadmin($userID)) die($_language->module['no_access']);

	$date=time();
	$paydate=strtotime($_POST['date']);

	safe_query("INSERT INTO ".PREFIX."cashbox ( date, paydate, usedfor, info, totalcosts, usercosts, squad, konto ) VALUES ('$date', '$paydate', '".$_POST['usedfor']."', '".$_POST['info']."', '".$_POST['euro']."', '".$_POST['usereuro']."', '".$_POST['squad']."', '".$_POST['konto']."' ) ");
...
elseif(isset($_POST['saveedit']) and $_POST['saveedit']) {
	...
	safe_query("UPDATE ".PREFIX."cashbox SET date='".$date."',
											  paydate='".$paydate."',
...
elseif(isset($_GET['delete']) and $_GET['delete']) {
	...
	if(!iscashadmin($userID)) die($_language->module['no_access']);
	$id = $_GET['id'];
	safe_query("DELETE FROM ".PREFIX."cashbox WHERE cashID='$id'");
	safe_query("DELETE FROM ".PREFIX."cashbox_payed WHERE cashID='$id'");
...
elseif(isset($_POST['pay']) and $_POST['pay']) {
	...
	foreach ( $payid as $usID => $costs ) {
		if($costs!="") {
			if(mysqli_num_rows(safe_query("SELECT payedID FROM ".PREFIX."cashbox_payed WHERE userID='$usID' AND cashID='$id'"))) {
				safe_query("UPDATE ".PREFIX."cashbox_payed SET costs='$costs' WHERE userID='$usID' AND cashID='$id'");
			}
			else {
				safe_query("INSERT INTO ".PREFIX."cashbox_payed (cashID, userID, costs, date, payed) VALUES ('$id', '$usID', '$costs', '$date', '1')");
			}
		}
	}
...
if(!isclanmember($userID) and !iscashadmin($userID)) echo $_language->module['clanmembers_only'];
	...
	if(isset($_GET['action']) and $_GET['action']=="new") {
		...
		if(!iscashadmin($userID)) die($_language->module['no_access']);
	    echo'<h2>'.$_language->module['cashbox'].'</h2>';
		...
		eval ("\$cashbox_new = \"".gettemplate("cashbox_new")."\";");
		echo $cashbox_new;
		...
		$id = $_GET['id'];
		$ergebnis=safe_query("SELECT * FROM ".PREFIX."cashbox WHERE cashID='$id'");
		...
		eval ("\$cashbox_edit = \"".gettemplate("cashbox_edit")."\";");
		echo $cashbox_edit;
		...
		function print_cashbox($squadID,$id) {
			...
			if($id) {
				$squadergebnis=safe_query("SELECT squad FROM ".PREFIX."cashbox WHERE cashID='".$id."'");
			...
			if($squadID == 0) $usersquad = $_language->module['clan'];
				...
				$ergebnis_squad = safe_query("SELECT * FROM ".PREFIX."cashbox_payed, ".PREFIX."cashbox WHERE ".PREFIX."cashbox_payed.payed='1' AND ".PREFIX."cashbox_payed.cashID=".PREFIX."cashbox.cashID AND ".PREFIX."cashbox.squad = '".$squadID."'");
				...
				$ergebnis_squad = safe_query("SELECT * FROM ".PREFIX."cashbox WHERE squad='$squadID'");
				$anz_squad = mysqli_num_rows($ergebnis_squad);
				if($anz_squad) {
					while($dss=mysqli_fetch_array($ergebnis_squad)) {
			...
			$ergebnis=safe_query("SELECT * FROM ".PREFIX."cashbox WHERE squad='".$squadID."' ORDER BY paydate DESC LIMIT 0,1");
			...
			if(mysqli_num_rows($ergebnis)) {
				...
				$bezahlen = safe_query("SELECT * FROM ".PREFIX."cashbox_payed WHERE cashID='$id' AND payed='1' ");
				...
				if(iscashadmin($userID)) $adminaction='<input type="button" onclick="MM_goToURL(\'parent\',\'index.php?site=cashbox&amp;action=edit&amp;id='.$id.'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" class="btn btn-danger"> <input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'cashbox.php?delete=true&amp;id='.$id.'\')" value="'.$_language->module['delete'].'" class="btn btn-danger">';

				eval ("\$cashbox_usage = \"".gettemplate("cashbox_usage")."\";");
				echo $cashbox_usage;

				$all=safe_query("SELECT * FROM ".PREFIX."cashbox WHERE squad='".$squadID."' ORDER BY paydate DESC");
				while($ds=mysqli_fetch_array($all)) {
					echo'&#8226; <a href="index.php?site=cashbox&amp;id='.$ds['cashID'].'&amp;squad='.$squadID.'"><b>'.$ds['usedfor'].'</b></a><br>';
				}
		        ...
		        eval ("\$cashbox_head = \"".gettemplate("cashbox_head")."\";");
				echo $cashbox_head;
        
				if(count($members)) {
					foreach($members as $usID) {
						$ergebnis = safe_query("SELECT * FROM ".PREFIX."cashbox_payed WHERE userID='$usID' AND cashID='$id'");
						...
						eval ("\$cashbox_content = \"".gettemplate("cashbox_content")."\";");
						echo $cashbox_content;
					}
				}
				...
				eval ("\$cashbox_foot = \"".gettemplate("cashbox_foot")."\";");
				echo $cashbox_foot;
		...
		$ergebnis = safe_query("SELECT * FROM ".PREFIX."cashbox_payed WHERE payed='1'");
		...
		$ergebnis = safe_query("SELECT * FROM ".PREFIX."cashbox ");
		$anz = mysqli_num_rows($ergebnis);
		if($anz) {
			while($ds=mysqli_fetch_array($ergebnis)) {
				$costs-=$ds['totalcosts'];
		...
		if(iscashadmin($userID)) $cashadmin='<input type="button" onclick="MM_goToURL(\'parent\',\'index.php?site=cashbox&amp;action=new\');return document.MM_returnValue" value="'.$_language->module['add_payment'].'" class="btn btn-danger">';


		eval ("\$cashbox_top = \"".gettemplate("cashbox_top")."\";");
		echo $cashbox_top;

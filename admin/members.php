...
if (isset($_GET[ 'action' ]) && $_GET[ 'action' ] == "edit") {
    ...
    if ($anz) {
      ...
      <tr>
        ...
        <td><input type="text" name="position[' . $ds[ 'sqmID' ] . ']" value="' . getinput($ds[ 'position' ]) .
                '" size="60" />' . $activity . '</td>
      </tr>
      <tr>
        <td><b>' . $_language->module[ 'access_rights' ] . '</b></td>
        <td>' . $_language->module[ 'joinus_admin' ] . ': ' . $join . '&nbsp; &nbsp; ' .
                $_language->module[ 'fightus_admin' ] . ': ' . $fight . '</td>
      </tr>';
        }
    }
...
onmouseout="hideWMTT()" />';
    ...
    <td colspan="2"><b>' . $_language->module[ 'description' ] . '</b><br />
        ...
        <br />
        <textarea id="message" rows="5" cols="" name="message" style="width: 100%;">' . getuserdescription($id) .
            '</textarea>
        </td>
  </tr>';
    } else {
    ...
    <form method="post" id="post" name="post" action="admincenter.php?site=members" onsubmit="return chkFormular();">
        ...
        <tr>
          ...
          <td width="85%"><a href="../index.php?site=profile&amp;id=' . $id . '" target="_blank">' .
            strip_tags(stripslashes(getnickname($id))) . '</a></td>
        </tr>
        ' . $squads . '
        ' . $userdes . '
        <tr>
    ...
    if (issuperadmin($userID)) {
        echo '<tr><td colspan="3">' . $super . ' <b>' . $_language->module[ 'super_admin' ] . '</b></td></tr>';
    }

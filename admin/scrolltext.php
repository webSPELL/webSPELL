...
$_language->readModule('scrolltext');
...
if (isset($_POST[ 'submit' ]) != "") {
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        if (
            mysqli_num_rows(
                safe_query("SELECT * FROM " . PREFIX . "scrolltext")
            )
        ) {
            safe_query(
                "UPDATE
        ...
        redirect("admincenter.php?site=scrolltext", $_language->module[ 'transaction_invalid' ], 3);
    }
    ...
    $direction = str_replace(
        'value="' . $ds[ 'direction' ] . '"',
        'value="' . $ds[ 'direction' ] . '" selected="selected"',
        $direction
    );
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="3">
        <tr>
            <td colspan="2"><b>' . $_language->module[ 'scrolltext' ] . '</b><br /><small>' .
        $_language->module[ 'you_can_use_html' ] . '</small><br /><br />
        ...
        <tr>
            ...
            <td><input type="text" name="color" value="' . $ds[ 'color' ] . '" maxlength="7" /> ' .
        $_language->module[ 'example' ] . '</td>
        </tr>
        <tr>
            <td colspan="2"><br /><input type="hidden" name="captcha_hash" value="' . $hash .

...
$_language->readModule('addons', false, true);
...
if (isset($_GET[ 'delete' ])) {
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        ...
        safe_query(
            "INSERT INTO " . PREFIX . "addon_links ( catID, name, url, accesslevel, sort )
            values (
            '" . $_POST[ 'catID' ] . "',
            '" . $_POST[ 'name' ] . "',
            '" . $_POST[ 'url' ] . "',
    ...
    if (
        ...
        safe_query(
            "INSERT INTO " . PREFIX . "addon_categories ( name, sort )
...
if ($action == "add") {
    echo '<h1>&curren; <a href="admincenter.php?site=addons" class="white">' . $_language->module[ 'addons' ] .
        '</a> &raquo; ' . $_language->module[ 'add_link' ] . '</h1>';
    ...
    echo '<h1>&curren; <a href="admincenter.php?site=addons" class="white">' . $_language->module[ 'addons' ] .
        '</a> &raquo; ' . $_language->module[ 'edit_link' ] . '</h1>';
    ...
    while ($ds = mysqli_fetch_array($ergebnis)) {
        ...
        $links =
            safe_query("SELECT * FROM " . PREFIX . "addon_links WHERE catID='" . $ds[ 'catID' ] . "' ORDER BY sort");
        $tmp = mysqli_fetch_assoc(
            safe_query(
                "SELECT count(linkID) as cnt
                  FROM " . PREFIX . "addon_links WHERE catID='" . $ds[ 'catID' ] . "'"
            )
        );
        $anzlinks = $tmp[ 'cnt' ];
    ...
    echo '	<tr>
                <td class="td_head" colspan="5" align="right"><input type="submit" name="sortieren" value="' .
